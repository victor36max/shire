...
describe("AgentForm", () => {
  ...
  it("removes a reference from a skill", async () => {
    renderForm();
    await userEvent.click(screen.getByText("Add Skill"));
    await userEvent.click(screen.getByText("Add Reference"));
    expect(screen.getByPlaceholderText("e.g. api-patterns.md")).toBeInTheDocument();

    // Find the Remove button inside the reference section (second Remove button)
    const removeButtons = screen.getAllByText("Remove");
    // removeButtons[0] = Remove skill, removeButtons[1] = Remove reference
    await userEvent.click(removeButtons[removeButtons.length - 1]);
    expect(screen.queryByPlaceholderText("e.g. api-patterns.md")).not.toBeInTheDocument();
  });

  it("updates reference fields", async () => {
    const user = userEvent.setup();
    renderForm();
    await user.click(screen.getByText("Add Skill"));
    await user.click(screen.getByText("Add Reference"));

    const refNameInput = screen.getByPlaceholderText("e.g. api-patterns.md");
    await user.type(refNameInput, "my-ref.md");
    expect(refNameInput).toHaveValue("my-ref.md");

    const refContentInput = screen.getByPlaceholderText("Reference content...");
    await user.type(refContentInput, "Some ref content");
    expect(refContentInput).toHaveValue("Some ref content");
  });

  it("updates skill fields individually", async () => {
    const user = userEvent.setup();
    renderForm();
    await user.click(screen.getByText("Add Skill"));

    const descInput = screen.getByPlaceholderText("When to use this skill...");
    await user.type(descInput, "Test desc");
    expect(descInput).toHaveValue("Test desc");

    const contentInput = screen.getByPlaceholderText("Markdown instructions...");
    await user.type(contentInput, "# My skill");
    expect(contentInput).toHaveValue("# My skill");
  });

  it("closes dialog when onClose is called", async () => {
    const onClose = mock(() => {});
    const { unmount } = render(
      <AgentForm open={true} title="New Agent" agent={null} onSave={onSave} onClose={onClose} />,
    );
  ...
  it("shows different description based on title", () => {
    render(
      <AgentForm
        open={true}
        title="Edit Agent"
    ...
    expect(screen.getByText("Update the agent details.")).toBeInTheDocument();
  });

  it("shows opencode model placeholder when harness is opencode", () => {
    const agent: Agent = {
      id: "a-oc",
      name: "oc-agent",
      status: "idle",
    ...
    expect(screen.getByText(/OpenCode requires provider\/model format/)).toBeInTheDocument();
  });
