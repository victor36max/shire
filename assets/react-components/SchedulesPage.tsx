...
function parseCronToForm(cron: string): Partial<ScheduleFormState> {
  ...
  if (dow && dow !== "*") {
    const days = dow.split(",").map((d) => {
      const idx = DAY_LABELS.findIndex((l) => l.toUpperCase().slice(0, 3) === d.toUpperCase().slice(0, 3));
      return idx + 1;
    });
    return {
      frequency: "weekly",
...
export default function SchedulesPage({ project, agents, tasks, pushEvent }: SchedulesPageProps) {
  ...
  const handleSubmit = () => {
    ...
    if (editingId) {
      pushEvent("update-schedule", { id: editingId, ...payload });
    } else {
      pushEvent("create-schedule", payload);
    }
  ...
  const handleDelete = () => {
    if (deleteId) {
      pushEvent("delete-schedule", { id: deleteId });
      setDeleteId(null);
    }
  };

  const handleToggle = (task: ScheduledTask) => {
    pushEvent("toggle-schedule", { id: task.id, enabled: !task.enabled });
  };

  const handleRunNow = (task: ScheduledTask) => {
    pushEvent("run-now", { id: task.id });
  };
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="sm:max-w-lg">
            ...
            {form.schedule_type === "recurring" && (
              <div className="space-y-4 rounded-md border p-4">
                <div className="space-y-2">
                  ...
                  <Select value={form.frequency} onValueChange={(v) => updateForm("frequency", v as Frequency)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmit} disabled={!form.label || !form.agent_id || !form.message}>
