defmodule Shire.AgentsTest do
  ...
  defp valid_recipe(name \\ "test-agent", opts \\ []) do
    harness = Keyword.get(opts, :harness, "claude_code")
  ...
  describe "messages" do
    ...
    test "list_inter_agent_messages/0 returns only inter_agent messages", %{agent: agent} do
      {:ok, _} =
        Agents.create_message(%{agent_id: agent.id, role: "user", content: %{"text" => "hi"}})
    ...
    test "list_inter_agent_messages/1 supports cursor-based pagination", %{agent: agent} do
      for i <- 1..3 do
        Agents.create_message(%{
          agent_id: agent.id,
          role: "inter_agent",
